...
final class Ararat {
	...
	private function setup() {
		...
		$enqueu          = new ararat_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->ararat_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'ararat_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'ararat', ARARAT_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'ararat_site_logo_123x40', 123, 40, true );

		// About thumb size
		add_image_size( 'ararat_about_thumb_585x750', 585, 750, true );

		// Service thumb size
		add_image_size( 'ararat_service_thumb_200x200', 200, 200, true );

		// Project thumb sizes
		add_image_size( 'ararat_project_thumb_585x450', 585, 450, true );
		add_image_size( 'ararat_project_thumb_484x700', 484, 700, true );

		// Client image size
		add_image_size( 'ararat_client_img_116x116', 116, 116, true );

		// Message image size
		add_image_size( 'ararat_message_img_585x700', 585, 700, true );

		// Latest post thumbnail size
		add_image_size( 'ararat_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'ararat_single_blog_750x375', 750, 375, true );
		add_image_size( 'ararat_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' 		=> esc_html__( 'Primary Menu', 'ararat' ),
			'navigation-menu' 	=> esc_html__( 'Navigation Menu', 'ararat' ),
			'services-menu' 	=> esc_html__( 'Services Menu', 'ararat' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = ARARAT_DIR_CSS_URI;
		$jsPath  = ARARAT_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->ararat_version,
				),
				array(
					'handler' => 'ararat-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'ararat' ) ) {
			
			$font_families = array(
				'Rubik:300,400,400i,500,500i,700,700i,900',
				'Playfair+Display:400,400i,700,900',
				'Roboto:300,300i,400,400i,500,700,700i',
			);
	...
	private function customizer_init(){
		...
		// Instantiate ararat theme customizer
		$ararat_theme_customizer = new ararat_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'ararat_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'ararat-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'ararat_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'ararat-elementor-default-style',  ARARAT_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->ararat_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'ararat_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'ararat-elementor-notice', ARARAT_DIR_JS_URI.'ararat-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'ararat-elementor-notice',
				'araratElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'ararat-elementor-notice-nonce' ),
				)
			);
		}
