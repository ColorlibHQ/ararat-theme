...
function ararat_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'ararat' ),
        'next_string'     => esc_html__( '&raquo;', 'ararat' ),
        'before_output'   => '<div class="ararat-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'ararat_pagination_defaults', $defaults )
    );
